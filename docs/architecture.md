...
- **Pros:** Incredibly fast. The return traffic from the backend can even bypass the load balancer entirely (Direct Server Return - DSR).
...
- **Cons:** Slower than passthrough. Data must be copied from the kernel space to user space (the Go application) and back to the kernel space for every packet, resulting in high CPU context-switching overhead.

*Note: This project implements **Proxy Mode** to prioritize learning dynamic routing, state management, and SRE resilience patterns.*
...
### eBPF and `sockmap`
eBPF (Extended Berkeley Packet Filter) allows us to run sandboxed programs safely within the Linux kernel. While ultra-high-performance load balancers (like Katran) use eBPF/XDP for *passthrough* routing at the network driver level, we can use a specific eBPF feature called `sockmap` to optimize our *proxy*.
...
This bypasses the Go user space entirely for the payload data transfer, drastically reducing CPU context switches while maintaining the flexible architecture of a Proxy-mode load balancer.
